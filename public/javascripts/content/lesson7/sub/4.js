...
function ComplexLogicalExpressionOr() {
	...
	function content() {
		...
		return '<p>Кадет сконцентрируйтесь. От пилота постоянно требуется следить и проверять несколько факторов одновременно.</p>' +
			   '<p>В таких случаях используются логические операторы. Рассмотрим один из них.</p>' +
			   '<p>Логический оператор <span class="under-label">|| (или)</span>  возвращает <strong>«true»</strong>, ' +
			   'если один или оба операнда возвращают <strong>«true»</strong>, иначе возвращает <strong>«false»</strong>. </p>' +
			   '<p>Но в языке программирования <strong>JavaScript</strong> не все так просто, об этом вы узнаете в следующих уроках. </p>' +
			   '<pre> ' +
			   'result = <strong>op1</strong> || <strong>op2</strong>' +
			   '</pre>' +
