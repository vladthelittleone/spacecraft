window.onload = function() {
	...
	var starsNumber = 800; //you can change the number of stars here
	var addedMass = 0; //number of stars eaten :D
	var holeRadius = 2;
	...
	function makeStar(new_star) {
		...
		if (new_star == 0) {
			...
			orbitRadius = Math.sqrt((C.width / 2 - C.width) * (C.width / 2 - C.width) +
						  			(C.height / 2 - C.height) * (C.height / 2 - C.height)) + Math.random() * 200;
		}
	...
	function drawCenter() {
		...
		if (holeRadius <= radiusLimit) {
			...
			holeRadius = 1 * Math.sqrt(addedMass / Math.PI) + 10;
	...
	function updateStar(i) {
		...
		star.color = "rgba(255," + Math.round(255 * ((star.orbitRadius - holeRadius) / 200)) + ","
								 + Math.round(255 * ((star.orbitRadius - holeRadius) / 200)) + ",.5)";
